...
def main():
    ...
    for i in range(len(dataframes)):
        dataframes[i] = dataframes[i].drop(columns=['original_category', 'original_is_safe'])

    # Join datasets on prompt/response pairs
    joined_dataframe = dataframes[0]
    for dataframe in dataframes[1:]:
        joined_dataframe = pd.merge(joined_dataframe, dataframe, on=['prompt', 'response'], how='inner')
