...
def main():
    parser = argparse.ArgumentParser(
        description="Clean a teacher model dataset for use in consensus"
    )
    ...
    for column in df.columns:
        if column.endswith("_safety_categories"):
            ...
            valid_categories = (
                MLC_TAXONOMY_0_5_ALIASES  # Replace with your valid category strings
            )
            df[column] = df[column].apply(
                lambda x: [item for item in x if item in valid_categories]
            )
